...
class TestDimension(unittest.TestCase):
    ...
    def test_string_representation(self):
        ...
        self.assertEqual(
            Dimension(M=2, L=1, T=-0.5).str(use_braces=True), "{M}^2{L}/{T}^0.5"
        )
        self.assertEqual(
    ...
    def test_parsing_elements(self):
        ...
        self.assertRaises(
            ParseException, e.parseString, "T^1/2"
        )  # fractions should not fail silently
    ...
    def test_parse_string_representation(self):
        ...
        self.assertEqual(
            parse_unit_string(Dimension(L=1, T=1).str(), unit_values), 3 * 4
        )
        self.assertEqual(
            parse_unit_string(Dimension(L=1, T=-1).str(), unit_values), 3 / 4
        )
        self.assertEqual(
            parse_unit_string(Dimension(L=1, T=-1, Theta=2).str(), unit_values),
            3 / 4 * 36,
        )
        ...
        self.assertEqual(
            parse_unit_string(Dimension(L=1, T=-0.5).str(), unit_values), 3 / 2
        )
        self.assertEqual(
            parse_unit_string(Dimension(M=2, L=1, T=-0.5).str(), unit_values), 4 * 3 / 2
        )
        self.assertEqual(
